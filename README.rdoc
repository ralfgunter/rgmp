...
1) My knowledge of math is pretty limited, and so I usually have to do some research or talk to someone before actually writing a test case (function domain, etc).
2) Most transcedental functions are not implemented natively in Ruby. For the time being, the result is checked against a short C program written specifically for the function being tested.
